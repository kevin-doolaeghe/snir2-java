...
public class ClientMaker extends JDialog implements ActionListener, FocusListener {
	...
	public ClientMaker(List<ClientPanel> clients, JTabbedPane tabbedPane) {
		...
		if (event.getSource() == button_apply) {
			try {
				...
				if (!textField_name.getText().isEmpty() && !textField_ip.getText().isEmpty()
					...
					ClientPanel client = new ClientPanel(socket, name, username);
					clients.add(client);
					tabbedPane.add(client);
